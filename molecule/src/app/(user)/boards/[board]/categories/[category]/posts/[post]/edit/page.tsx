...
export default function Page({ params }: { params: { board: string; category: string; post: string } }) {
  ...
  if (isError) {
    ...
    router.push(`/boards/${params.board}/categories/${params.category}/`);
    return <></>;
  }

  return (
    <>
      <h2 className="text-xl font-bold text-gray-800 dark:text-gray-300 md:text-2xl">글 수정</h2>
      <PostForm mode={"edit"} boardId={params.board} post={post} />
    </>
  );
}
