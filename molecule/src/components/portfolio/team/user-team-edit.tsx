...
import { useTeam } from "@/store/queries/portfolio/portfolio";
...
const UserWorkEdit = ({ portfolioId, id }: { portfolioId: string; id?: string }) => {
  ...
  const { data: team, refetch } = useTeam(portfolioId, id ?? "", {
    enabled: id !== null,
  });
  ...
  useEffect(() => {
    if (team) {
      const startDate = team.start_date.split("-");
      const endDate = team.end_date.split("-");
      ...
      form.reset({
        ...
        end_year: endDate[0],
        end_month: endDate[1],
        link: team.links.link,
      });
      setCheck(team.is_current);
      setSelectedSkill(team.skills);
    }
  }, [form, team]);
          ...
          >
            ...
            <div className="w-full space-y-2">
              <div className="flex items-center">
                <span className="text-sm font-semibold md:text-base">소속 기간</span>
